...
1. Wrap the Project section in a sed-strippable comment in root CLAUDE.md:
   ```markdown
   <!-- REMOVE-FOR-NPM-PUBLISH: This section is a template for monorepo root only. Published methodology users fill this after `npx voidforge-build init`. -->
   ## Project
   - **Name:** [PROJECT_NAME]
   ...
...
3. `npx voidforge-build init` prompts for Project metadata and writes it to the user's local CLAUDE.md.
